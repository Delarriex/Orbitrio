...
// prices (Binance → CoinGecko → mock fallback) and, using the Supabase
// service_role key (server-only), upserts the prices into `market_prices` so
// the buy_asset/sell_asset RPCs have a trusted source (bug #22). Without this
// running in production, trading would reject with "No market price available".
